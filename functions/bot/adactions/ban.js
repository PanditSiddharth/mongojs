...
const unmt = async (bot, ctxx, userId, name) => {
    ...
    if (ctxx.state.rmem.status == 'creator') {
        let k = await bot.telegram.sendMessage(ctxx.chat.id, `[${name}](tg://user?id=${userId}) is Owner I can't ban them`, { parse_mode: "Markdown" });
        await ctxx.state.sleep(1)
        return await bot.telegram.editMessageText(ctxx.chat.id, k.message_id, undefined, `${name} is Owner and I can't ban them`);
    }
    ...
    if (userId == m.id)
        return await ctxx.reply("You can't ban me lol")

    /* ************************************************************************************** */


    if (ctxx.state.rmem.status == 'administrator') {
        let k = await bot.telegram.sendMessage(ctxx.chat.id, `[${name}](tg://user?id=${userId}) is admin, I can't ban...`, { parse_mode: "Markdown" });
        await ctxx.state.sleep(1)
        return await bot.telegram.editMessageText(ctxx.chat.id, k.message_id, undefined, `${name} is admin, I can't ban...`);
    }

    /* ************************************************************************************** */

    if (ctxx.state.rmem.status == 'kicked')
        return await ctxx.reply(name + " is already banned")

    /* ************************************************************************************** */

    try {
        let n = await bot.telegram.banChatMember(ctxx.chat.id, userId)
        ...
        if (n == true) {
            let k = await bot.telegram.sendMessage(ctxx.chat.id, `User [${name}](tg://user?id=${userId}) is Banned`, { parse_mode: "Markdown" });
            await ctxx.state.sleep(1)
            return await bot.telegram.editMessageText(ctxx.chat.id, k.message_id, undefined, `User ${name} is Banned`);
        }
