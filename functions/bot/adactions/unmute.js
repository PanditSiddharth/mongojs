...
const unmute = async (bot, ctxx) => {
    ...
    try {
        ...
        if (ctxx.message.entities[1] && ctxx.message.entities[1].type == "text_mention") {
            console.log(ctxx.message.entities)
        ...
        /* ************************************************************************************** */
        ...
        else if(ctxx.message.entities || ctxx.message.entities[1].type == 'mention'){
            const t = ctxx.message.entities[1]
            const userId = ctxx.message.text.substring(t.offset, t.length + t.offset)
            const name = ctxx.message.text.substring(t.offset, t.length + t.offset)
            ...
            return ctxx.reply('Please reply to a message from the user you want to unmute');
        }
...
const unmt = async (bot, ctxx, userId, name, yes = true) => {
    ...
    try {
        ...
        if (n == true){
            let k = await bot.telegram.sendMessage(ctxx.chat.id, `[.](tg://user?id=${userId}) now you can speak`, { parse_mode: "Markdown" });
            await ctxx.state.sleep(1)
